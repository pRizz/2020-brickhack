(ns brickhack.intersections-flannel
...
(defn sketch-update [particles]
  (->> particles
       (map-indexed
         (fn [index particle]
           (let [particle-section (Math/floor (/ index default-particle-count))
                 v-multiplier (if (even? particle-section) -1 1)
                 direction-multiplier (if (>= index (* 2 default-particle-count)) 0.5 1)]
             (assoc particle
